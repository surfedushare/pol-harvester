    ...
    with io.open(dic, 'r') as Fdic:
        for line in Fdic:
            ...
            if word not in words:
                ...
                words[word].add(pronunciation)

# it felt weird that new pronounciations were sorted into the list, but completely new words weren't
od = collections.OrderedDict(sorted(words.items()), key=lambda x:x[1], revers=True)
